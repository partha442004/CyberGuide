...
def show_settings() -> None:
    ...
    # One digest per day (08:00 IST) — the per-slot category pickers and the
    # weekly recap toggle were retired with the 3-slot schedule. slot_domains
    # is still saved (harmless legacy data) but no longer drives sends.
    ...
    with col1:
        if st.button("💾 Save Alert Preferences", use_container_width=True):
            ...
            if result:
                ...
                st.success(
                    f"✅ Preferences saved! Your daily digest (8 AM IST) sends "
                    f"**{cats}** via "
                    f"{', '.join(result.get('channels') or payload['channels'])}."
                )
    ...
    if domains:
        st.caption(
            "🔔 Your daily digest (8 AM IST) is filtered to: "
