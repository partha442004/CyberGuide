"""
Unit Tests for McAfee Scraper

Tests the McAfeeScraper class covering:
...
class TestMcAfeeScraper:
    ...
    def test_build_search_payload_pagination(self):
        ...
        payload = self.scraper._build_search_payload("SOC", page=3, limit=15)
        assert payload["offset"] == 45
    ...
    def test_parse_job_data(self):
        ...
        assert job.company_name == "McAfee"
        assert job.source == "company_mcafee"
    ...
    def test_tag_job(self):
        ...
        assert tagged.company_name == "McAfee"
        assert tagged.source == "company_mcafee"

    def test_is_security_role_positive(self):
        """Should detect security roles."""
        job = ScrapedJob()
        job.title = "Endpoint Security Engineer"
        job.description = "Building endpoint protection tools"
...
class TestMcAfeeScraperConfig:
    ...
    def test_custom_config(self):
        ...
        scraper = McAfeeScraper(config=config)
        assert scraper.config.name == "custom_mcafee"
