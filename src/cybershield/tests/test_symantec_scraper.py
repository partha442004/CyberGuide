"""
Unit Tests for Symantec (Broadcom) Scraper

Tests the SymantecScraper class covering:
...
class TestSymantecScraper:
    ...
    def test_default_keywords(self):
        ...
        assert any("endpoint" in kw.lower() for kw in keywords)
    ...
    def test_build_search_payload_pagination(self):
        ...
        payload = self.scraper._build_search_payload("DLP", page=2, limit=10)
    ...
    def test_parse_job_data(self):
        ...
        assert job.company_name == "Broadcom Symantec"
        assert job.source == "company_broadcom_symantec"
        assert job.location == "Austin, TX (US)"
        assert job.country == "USA"
        assert job.job_type == "full_time"
        assert "broadcom" in (job.url or "").lower()
    ...
    def test_tag_job(self):
        ...
        assert tagged.company_name == "Broadcom Symantec"
        assert tagged.source == "company_broadcom_symantec"
...
class TestSymantecScraperConfig:
    """Test scraper configuration."""

    def test_default_config(self):
        """Should create default config correctly."""
        scraper = SymantecScraper()
    ...
    def test_custom_config(self):
        ...
        config = ScraperConfig(
            name="custom_symantec",
            base_url="https://custom.url",
            rate_limit=0.5,
            max_retries=5,
        )
        scraper = SymantecScraper(config=config)
        assert scraper.config.name == "custom_symantec"
