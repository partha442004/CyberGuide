"""
Hired scraper for job market.

Scrapes cybersecurity jobs from Hired.com (tech-focused job platform).
"""
...
class HiredScraper(BaseScraper):
    """Scraper for Hired.com job postings."""
    ...
    async def fetch(
        ...
        try:
            ...
            logger.error(f"Error fetching from Hired: {e}")

        return jobs

    def _parse_job_card(self, card) -> RawJob | None:
        """Parse a job card from Hired HTML."""
        try:
            # Extract title
            ...
            return RawJob(
                title=title,
                company=company,
                url=url or f"https://hired.com/jobs?q={title}",
                location=location,
