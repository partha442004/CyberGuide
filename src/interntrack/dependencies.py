...
async def get_job_service(db: AsyncSession = None) -> JobService:
    """Get job service."""
    if db is None:
        async with get_db_session() as session:
...
async def get_application_service(db: AsyncSession = None) -> ApplicationService:
...
async def get_notification_manager(db: AsyncSession = None) -> NotificationManager:
...
async def get_report_service(db: AsyncSession = None) -> ReportService:
    """Get report service."""
    if db is None:
        async with get_db_session() as session:
...
async def get_ai_service(db: AsyncSession = None) -> AIService:
    """Get AI service."""
    if db is None:
        async with get_db_session() as session:
