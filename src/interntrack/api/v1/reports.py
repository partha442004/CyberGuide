...
async def _top_engaged_jobs(db, days: int = 7, limit: int = 5) -> list[dict]:
    ...
    Delegates to :meth:`JobRepository.get_most_engaged` — the single
    source of truth also used by the scheduler's weekly digest, so the
    engagement formula can never drift between the two call sites.
    Returns plain dicts; never raises.
    """
    try:
        from interntrack.repositories.job_repository import JobRepository

        return await JobRepository(db).get_most_engaged(days=days, limit=limit)
