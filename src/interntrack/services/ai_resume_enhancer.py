...
class AIResumeEnhancer:
    ...
    def enhance_skill_extraction(self, text: str, basic_skills: list[dict]) -> list[dict]:
        ...
        enhanced_skills = self._boost_confidence(enhanced_skills, text)
        ...
        for category, patterns in CONTEXT_PATTERNS.items():
            for pattern in patterns:
                ...
                for match in matches:
                    ...
                    if skill_name and skill_name not in [s["name"].lower() for s in skills]:
                        skills.append({
    ...
    def _infer_skill_from_context(self, context: str, category: str) -> str | None:
        ...
        category_skills = {
            "penetration_testing": ["penetration testing", "vulnerability assessment", "ethical hacking"],
            "soc_analyst": ["soc", "siem", "incident response", "log analysis"],
            "malware_analysis": ["malware analysis", "reverse engineering", "forensics"],
    ...
    def _extract_synonym_skills(self, text: str) -> list[dict]:
        ...
        for synonym, standard_name in SKILL_SYNONYMS.items():
            if synonym in text_lower and standard_name not in [s["name"].lower() for s in skills]:
    ...
    def extract_preferred_roles(self, text: str) -> list[str]:
        ...
        # Role keywords
        role_patterns = [
            (r"(?:seeking|looking for|interested in|pursuing)\s+(?:a\s+)?(.*?)(?:\s+position|\s+role|\s+job|\s+career|\.|,|$)", 1),
        ...
        for pattern, group in role_patterns:
            ...
            for match in matches:
                ...
                role_titles = [
                    ...
                    "malware analyst", "forensics analyst", "incident responder",
                    "cloud security engineer", "application security engineer",
                    "network security engineer", "security auditor",
                ]
...
def enhance_resume_parsing(text: str, basic_skills: list[dict]) -> dict[str, Any]:
    ...
    return {
        ...
        "high_confidence_skills": len([s for s in enhanced_skills if s.get("confidence", 0) >= 0.85]),
    }
