...
from interntrack.domain.models import Skill, UserSkill
...
class JobRecommender:
    ...
    async def get_personalized_recommendations(
        ...
        recommendations.sort(key=lambda x: float(str(x["match_score"])), reverse=True)
    ...
    async def _get_user_preferences(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("Could not load user preferences for %s: %s", user_id, e)
    ...
    def _calculate_job_score(
        ...
        location_score = self._calculate_location_score(
            job, user_profile, location_filter
        )
    ...
    def _calculate_experience_match(self, job: Any, user_profile: dict) -> float:
        ...
        if any(
            kw in title_lower
            for kw in ["senior", "lead", "principal", "staff", "architect"]
        ):
            job_level = "senior"
        elif any(
            kw in title_lower
            for kw in ["junior", "entry", "associate", "fresher", "intern"]
        ):
            job_level = "junior"
        ...
        avg_proficiency = (
            sum(s["proficiency"] for s in user_skills.values()) / len(user_skills)
            if user_skills
            else 3
        )
    ...
    def _calculate_recency_score(self, job: Any) -> float:
        ...
        if isinstance(posted_at, datetime):
            days_ago = (datetime.now(UTC) - posted_at).days
        else:
            return 50.0
