...
class TestRunJobDiscovery:
    ...
    async def test_run_job_discovery(self):
        ...
        with patch("interntrack.scheduler.jobs.get_db_session") as mock_db, \
            ...
            with patch("interntrack.scrapers.registry.get_default_registry", return_value=mock_registry):
...
class TestGenerateDailyReport:
    ...
    async def test_generate_daily_report(self):
        ...
        mock_report_service.generate_daily_report = AsyncMock(
            return_value={
                "report_type": "daily",
                "summary": {"new_jobs": 5, "new_applications": 2, "total_applications": 10},
            }
        )
        ...
        with patch("interntrack.scheduler.jobs.get_db_session") as mock_db, \
             patch("interntrack.scheduler.jobs.ReportService", return_value=mock_report_service), \
             patch("interntrack.scheduler.jobs.NotificationManager", return_value=mock_manager):
            mock_db.return_value.__aenter__ = AsyncMock(return_value=mock_session)
            mock_db.return_value.__aexit__ = AsyncMock(return_value=False)
...
class TestVerifyJobLinks:
    ...
    async def test_verify_job_links(self):
        ...
        with patch("interntrack.scheduler.jobs.get_db_session") as mock_db:
            ...
            with patch("interntrack.engines.verification.VerificationEngine", return_value=mock_engine):
