...
class TestClassificationEngineExtended:
    ...
    async def test_extract_skills_deduplication(self):
        ...
        python_calls = [
            c
            for c in engine.skill_repo.create_or_get.call_args_list
            if c[0][0] == "python"
        ]
...
class TestDeduplicationEngineExtended:
    ...
    async def test_find_duplicates_in_database(self):
        ...
        job1 = MagicMock()
        job1.id, job1.title, job1.company, job1.url = (
            "j1",
            "Python Developer",
            "TechCorp",
            "https://example.com/1",
        ...
        job2 = MagicMock()
        job2.id, job2.title, job2.company, job2.url = (
            "j2",
            "Python Developer",
            "TechCorp",
            "https://example.com/2",
        ...
        job3 = MagicMock()
        job3.id, job3.title, job3.company, job3.url = (
            "j3",
            "Java Developer",
            "OtherCorp",
...
class TestVerificationEngineExtended:
    ...
    def test_validate_salary_normal(self):
        engine = _make_verification_engine()
        assert (
            engine._validate_salary({"salary_min": 50000, "salary_max": 100000}) == []
        )
