...
- **Version**: both packages single-source-of-truth at **1.18.0** — `app_version` reads package `__version__` (interntrack + cybershield), synced with .env/.env.example and root `pyproject.toml`; canary tests in both suites + `scripts/check_versions.py` CI gate
...
| **Tests** | ✅ Complete | 100% | 827 tests passing |
...
### Tests (827 total: 504 InternTrack + 323 CyberGuide)
